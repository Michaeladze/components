...
var FileInput = function (_a) {
    ...
    var onChange = function () {
        if (ref.current && ref.current.files) {
            var promises_1 = [];
            Array.from(ref.current.files).forEach(function (fl) {
                var validationResult = file_utils_1.validateFile(fl, {
                    maxSize: maxSize,
                    accept: accept
                });
                if (validationResult.valid) {
                    promises_1.push(file_utils_1.getneutral64(fl));
                }
                else {
            ...
            Promise.all(promises_1)
                ...
                if (data && ref.current && ref.current.files) {
                    ...
                    if (multiple) {
                        var keysMap_1 = {};
                        __spreadArrays(file, data).forEach(function (f) {
