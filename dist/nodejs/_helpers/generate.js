...
var generate = function (_filename, _components, _project, _path) {
    ...
    _components.split(',').forEach(function (action) {
        ...
        if (fs.existsSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".actions.ts"])))) {
            var data = fs.readFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".actions.ts"])), 'utf-8');
            ...
            if (~data.indexOf("[Pending] " + action)) {
                ...
                fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".actions.ts"])), (d[0] + commonAction1 + commonActionTmp + d[1] + commonAction2 + commonActionTmp1).replaceAll('$$$name$$$', action));
            }
        }
        else {
            fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".actions.ts"])), (commonActionLib + commonAction1 + commonActionTmp + commonAction2 + commonActionTmp1).replaceAll('$$$name$$$', action));
        }
    });
    //======================================================================================================================
    ...
    _components.split(',').forEach(function (reducer) {
        ...
        if (fs.existsSync(path.resolve.apply(path, __spreadArrays(reducersPath, [_filename + ".reducer.ts"])))) {
            var data = fs.readFileSync(path.resolve.apply(path, __spreadArrays(reducersPath, [_filename + ".reducer.ts"])), 'utf-8');
            ...
            if (~data.indexOf("[a." + reducer + ".Success]")) {
                ...
                fs.writeFileSync(path.resolve.apply(path, __spreadArrays(reducersPath, [_filename + ".reducer.ts"])), (d[0] + commonReducersTmp + commonReducersTemplate + d[1] + commonReducersTmp1 + '\n' + commonReducersExport).replaceAll('$$$name$$$', reducer));
            }
        }
        else {
            fs.writeFileSync(path.resolve.apply(path, __spreadArrays(reducersPath, [_filename + ".reducer.ts"])), (commonReducersLib + commonReducersRedInterface
    ...
    _components.split(',').forEach(function (action) {
        ...
        if (fs.existsSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".effects.ts"])))) {
            var data = fs.readFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".effects.ts"])), 'utf-8');
            if (~data.indexOf("export const " + actionl)) {
                console.warn(chalk.yellow("\u042D\u0444\u0444\u0435\u043A\u0442 " + actionl + "Effect$ \u0443\u0436\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442"));
            }
            else {
                fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".effects.ts"])), (data + commonEffect).replaceAll('$$$name$$$', action).replaceAll('$$$namel$$$', actionl));
            }
        }
        else {
            fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".effects.ts"])), (commonEffectLib + commonEffectSep + commonEffect)
    ...
    _components.split(',').forEach(function (action) {
        ...
        if (fs.existsSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".service.ts"])))) {
            var data = fs.readFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".service.ts"])), 'utf-8');
            if (~data.indexOf("export const " + action)) {
                console.warn(chalk.yellow("\u0421\u0435\u0440\u0432\u0438\u0441 " + action + " \u0443\u0436\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442"));
            }
            else {
                fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".service.ts"])), (data + commonService).replaceAll('$$$name$$$', action));
            }
        }
        else {
            fs.writeFileSync(path.resolve.apply(path, __spreadArrays(componentPath, [_filename + ".service.ts"])), (commonServiceLib + commonService).replaceAll('$$$name$$$', action));
        }
    });
    var content = '';
